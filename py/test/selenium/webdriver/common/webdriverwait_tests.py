...
def throw_sere(driver: WebDriver) -> NoReturn:
    raise StaleElementReferenceException("test")


def test_should_fail_with_invalid_selector_exception(driver: WebDriver, pages: Pages) -> None:
    pages.load("dynamic.html")
    with pytest.raises(InvalidSelectorException):
        WebDriverWait(driver, 0.7).until(EC.presence_of_element_located((By.XPATH, "//*[contains(@id,'something'")))


def test_should_explicitly_wait_for_a_single_element(driver: WebDriver, pages: Pages) -> None:
    pages.load("dynamic.html")
...
def test_should_still_fail_to_find_an_element_with_explicit_wait(driver: WebDriver, pages: Pages) -> None:
    pages.load("dynamic.html")
    with pytest.raises(TimeoutException):
        WebDriverWait(driver, 0.7).until(EC.presence_of_element_located((By.ID, "box0")))


def test_should_explicitly_wait_until_at_least_one_element_is_found_when_searching_for_many(driver: WebDriver, pages: Pages) -> None:
    pages.load("dynamic.html")
    add = driver.find_element(By.ID, "adder")
...
def test_should_fail_to_find_elements_when_explicit_waiting(driver: WebDriver, pages: Pages) -> None:
    pages.load("dynamic.html")
    with pytest.raises(TimeoutException):
        WebDriverWait(driver, 0.7).until(EC.presence_of_all_elements_located((By.CLASS_NAME, "redbox")))


def test_should_wait_until_at_least_one_visible_elements_is_found_when_searching_for_many(driver: WebDriver, pages: Pages) -> None:
...
def test_should_fail_to_find_visible_elements_when_explicit_waiting(driver: WebDriver, pages: Pages) -> None:
    pages.load("hidden_partially.html")
    with pytest.raises(TimeoutException):
        WebDriverWait(driver, 0.7).until(EC.visibility_of_any_elements_located((By.CLASS_NAME, "redbox")))


def test_should_wait_until_all_visible_elements_are_found_when_searching_for_many(driver: WebDriver, pages: Pages) -> None:
    pages.load("hidden_partially.html")
    add_visible = driver.find_element(By.ID, "addVisible")
...
def test_should_fail_if_not_all_elements_are_visible(driver: WebDriver, pages: Pages) -> None:
    pages.load("hidden_partially.html")
    add_visible = driver.find_element(By.ID, "addVisible")
    add_hidden = driver.find_element(By.ID, "addHidden")
...
def test_should_wait_only_as_long_as_timeout_specified_when_implicit_waits_are_set(driver: WebDriver, pages: Pages) -> None:
    pages.load("dynamic.html")
...
def test_should_wait_at_least_once(driver: WebDriver, pages: Pages) -> None:
    pages.load("simpleTest.html")
    elements = WebDriverWait(driver, 0).until(lambda d: d.find_elements(By.TAG_NAME, 'h1'))
    assert len(elements) >= 1


def test_wait_until_not_returns_if_evaluates_to_false(driver: WebDriver, pages: Pages) -> None:
    assert WebDriverWait(driver, 1).until_not(lambda d: False) is False


def test_wait_should_still_fail_if_produce_ignored_exception(driver: WebDriver, pages: Pages) -> None:
...
def test_wait_should_still_fail_if_produce_child_of_ignored_exception(driver: WebDriver, pages: Pages) -> None:
    ignored = (WebDriverException)
    with pytest.raises(TimeoutException):
        WebDriverWait(driver, 1, 0.7, ignored_exceptions=ignored).until(throw_sere)


def test_wait_until_not_should_not_fail_if_produce_ignored_exception(driver: WebDriver, pages: Pages) -> None:
    ignored = (InvalidElementStateException, StaleElementReferenceException)
    assert WebDriverWait(driver, 1, 0.7, ignored_exceptions=ignored).until_not(throw_sere)


def test_expected_condition_title_is(driver: WebDriver, pages: Pages) -> None:
...
def test_expected_condition_title_contains(driver: WebDriver, pages: Pages) -> None:
    pages.load("blank.html")
    driver.execute_script("setTimeout(function(){document.title='not blank'}, 200)")
...
def test_expected_condition_visibility_of_element_located(driver: WebDriver, pages: Pages) -> None:
...
@pytest.mark.xfail_safari
def test_expected_condition_visibility_of(driver: WebDriver, pages: Pages) -> None:
    pages.load("javascriptPage.html")
...
def test_expected_condition_text_to_be_present_in_element(driver: WebDriver, pages: Pages) -> None:
...
def test_expected_condition_text_to_be_present_in_element_value(driver: WebDriver, pages: Pages) -> None:
    pages.load('booleanAttributes.html')
    with pytest.raises(TimeoutException):
...
def test_expected_condition_text_to_be_present_in_element_attribute(driver: WebDriver, pages: Pages) -> None:
    pages.load('booleanAttributes.html')
    with pytest.raises(TimeoutException):
...
def test_expected_condition_frame_to_be_available_and_switch_to_it_by_locator(driver: WebDriver, pages: Pages) -> None:
    pages.load("blank.html")
    with pytest.raises(TimeoutException):
...
def test_expected_condition_invisiblity_of_element(driver: WebDriver, pages: Pages) -> None:
    pages.load("javascriptPage.html")
...
def test_expected_condition_invisiblity_of_element_located(driver: WebDriver, pages: Pages) -> None:
    pages.load("javascriptPage.html")
    driver.execute_script("delayedShowHide(0, true)")
    with pytest.raises(TimeoutException):
...
@pytest.mark.xfail_safari
def test_expected_condition_element_to_be_clickable(driver: WebDriver, pages: Pages) -> None:
    pages.load("javascriptPage.html")
    with pytest.raises(TimeoutException):
...
def test_expected_condition_staleness_of(driver: WebDriver, pages: Pages) -> None:
...
def test_expected_condition_element_to_be_selected(driver: WebDriver, pages: Pages) -> None:
...
def test_expected_condition_element_located_to_be_selected(driver: WebDriver, pages: Pages) -> None:
    pages.load("formPage.html")
    element = driver.find_element(By.ID, 'checky')
    with pytest.raises(TimeoutException):
...
def test_expected_condition_element_selection_state_to_be(driver: WebDriver, pages: Pages) -> None:
    pages.load("formPage.html")
    element = driver.find_element(By.ID, 'checky')
...
def test_expected_condition_element_located_selection_state_to_be(driver: WebDriver, pages: Pages) -> None:
    pages.load("formPage.html")
    element = driver.find_element(By.ID, 'checky')
...
def test_expected_condition_alert_is_present(driver: WebDriver, pages: Pages) -> None:
    pages.load('blank.html')
    with pytest.raises(TimeoutException):
...
def test_expected_condition_attribute_to_be_include_in_element(driver: WebDriver, pages: Pages) -> None:
    pages.load('booleanAttributes.html')
    with pytest.raises(TimeoutException):
